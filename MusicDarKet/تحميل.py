...
**🌵 𓏺طَݪبَ مِنِ اެݪحِݪۅٛ مِاެݪيَ :** {message.from_user.mention}
"""
...
@Client.on_message(filters.command(["تنزيل فيديو", "ت فيديو"], prefixes=f"{HNDLR}"))
async def vsong(client, message: Message):
    urlissed = get_text(message)

    pablo = await client.send_message(message.chat.id, f"**❤️‍🔥 𓏺يَتَمِ اެݪبَحِثَ اެݪاެنِ** `{urlissed}`")
    if not urlissed:
    ...
    fridayz = mio[0]["id"]
    mio[0]["channel"]
    kekme = f"https://te.legra.ph/file/28519035fc9c98b9263c6.jpg"
    await asyncio.sleep(0.6)
    url = mo
    sedlyf = wget.download(kekme)
    ...
    capy = f"""
** 𓏺اެسِمِ اެݪفَيَدَيَۅٛ :** [{thum}]({mo})
**🧸♥️ 𓏺طَݪبَ مِنِ اެݪحِݪۅٛ مِاެݪيَ :** {message.from_user.mention}
"""
